...
export const fetchUsers = (page: number, searchTerm: string = "") => async (dispatch: AppDispatch) => {
...
export const fetchUser = (id: string) => async (dispatch: AppDispatch) => {
  try {
    const response = await axios.get(`${ACCOUNTS_URL}/users/${id}/`, getConfig());
...
export const createUser = (newUser: User) => async (dispatch: AppDispatch) => {
  ...
  try {
    const response = await axios.post(`${ACCOUNTS_URL}/users/`, newUser, getConfig());
...
export const updateUser = (id: string, updatedUser: User) => async (dispatch: AppDispatch) => {
  dispatch(updateUserStart());
  try {
    const response = await axios.put(`${ACCOUNTS_URL}/users/${id}/`, updatedUser, getConfig());
...
export const deleteUser = (id: string) => async (dispatch: AppDispatch) => {
  ...
  try {
    const response = await axios.delete(`${ACCOUNTS_URL}/users/${id}/`, getConfig());
