...
export const fetchCategories = (page: number, searchTerm: string = "") => async (dispatch: AppDispatch) => {
    ...
    try {
        ...
        dispatch(fetchCategoriesFailure({
            ...
            detailCode: error.response?.data.detail_code || 'FETCH_CATEGORIES_ERROR'
        }));
    }
};
...
export const fetchCategory = (id: string) => async (dispatch: AppDispatch) => {
    try {
        const response = await axios.get(`${INVENTORY_URL}/categories/${id}/`, getConfig());
...
export const createCategory = (newCategory: Category) => async (dispatch: AppDispatch) => {
    ...
    try {
        const response = await axios.post(`${INVENTORY_URL}/categories/`, newCategory, getConfig());
        ...
        dispatch(createCategoryFailure({
            ...
            status: error.response?.status || 500,
            detailCode: error.response?.data.detail_code || 'CREATE_CATEGORY_ERROR'
        }));
    }
};
...
export const updateCategory = (id: string, updatedCategory: Category) => async (dispatch: AppDispatch) => {
    ...
    try {
        const response = await axios.put(`${INVENTORY_URL}/categories/${id}/`, updatedCategory, getConfig());
        ...
        dispatch(updateCategoryFailure({
            ...
            errors: error.response?.data.errors || {},
            status: error.response?.status || 500,
            detailCode: error.response?.data.detail_code || 'UPDATE_CATEGORY_ERROR'
        }));
    }
};
...
export const deleteCategory = (id: string) => async (dispatch: AppDispatch) => {
    ...
    try {
        const response = await axios.delete(`${INVENTORY_URL}/categories/${id}/`, getConfig());
...
export const fetchSimplifiedCategories = (searchTerm: string = "") => async (dispatch: AppDispatch) => {
    ...
    try {
        ...
        dispatch(fetchSimplifiedCategoriesSuccess({
            message: response.data.message,
            data: response.data.categories,
            status: response.status,
            detailCode: response.data.detail_code
        ...
        dispatch(fetchSimplifiedCategoriesFailure({
            error: error.response?.data.message || error.message,
            status: error.response?.status || 500,
            detailCode: error.response?.data.detail_code || 'FETCH_SIMPLIFIED_CATEGORIES_ERROR'
        }));
    }
};
