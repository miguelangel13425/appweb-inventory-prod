    ...
    try {
      ...
      dispatch(
        fetchWarehousesFailure({
          ...
          detailCode:
            error.response?.data.detail_code || 'FETCH_WAREHOUSES_ERROR',
        }),
      )
    }
  }

export const fetchSimplifiedWarehouses =
  (searchTerm: string = '') =>
  async (dispatch: AppDispatch) => {
      ...
      dispatch(
        fetchSimplifiedWarehousesFailure({
          ...
          detailCode:
            ...
            'FETCH_SIMPLIFIED_WAREHOUSES_ERROR',
        }),
      )
    }
  }
...
export const createWarehouse =
  (newWarehouse: WarehouseForm) => async (dispatch: AppDispatch) => {
    dispatch(createWarehouseStart())
    try {
      const response = await axios.post(
        `${INVENTORY_URL}/warehouses/`,
        newWarehouse,
        getConfig(),
      )
      dispatch(
      ...
      dispatch(
        createWarehouseFailure({
          ...
          status: error.response?.status || 500,
          detailCode:
            error.response?.data.detail_code || 'CREATE_WAREHOUSE_ERROR',
        }),
      )
    }
  }
...
export const updateWarehouse =
  (id: string, updatedWarehouse: WarehouseForm) =>
  async (dispatch: AppDispatch) => {
    ...
    try {
      ...
      dispatch(
        updateWarehouseFailure({
          ...
          status: error.response?.status || 500,
          detailCode:
            error.response?.data.detail_code || 'UPDATE_WAREHOUSE_ERROR',
        }),
      )
    }
  }
    ...
    try {
      ...
      dispatch(
        deleteWarehouseFailure({
          ...
          status: error.response?.status || 500,
          detailCode:
            error.response?.data.detail_code || 'DELETE_WAREHOUSE_ERROR',
        }),
      )
    }
  }
